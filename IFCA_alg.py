...
def init_net_sever(args):
    ...
    if args.model == "lenet5" and args.dataset == "purchase100":
        ...
        for _ in range(args.n_clients):
            torch.manual_seed(0)
            users_model.append(
                PurchaseClassifier(nclasses=100, cfg=None, ks=args.ks).to(args.device)
            )
    if args.model == "lenet5" and (args.dataset == "cifar10" or args.dataset == "svhn"):
...
def run_task(args):
    ...
    sever_model_pool = init_net_sever(args)
    ...
    chosen_model_map = [-1 for _ in range(args.n_clients)]
    while current_round != 0:
        ...
        for idx in range(args.n_clients):
            ...
            loss_list_temp = []
            begin = time.time()
            for model in sever_model_pool:
                loss, _ = clients[idx].eval_test(model)
                loss_list_temp.append(loss)
            chosen_model_index = loss_list_temp.index(min(loss_list_temp))
            chosen_model = copy.deepcopy(sever_model_pool[chosen_model_index])
            clients[idx].net = chosen_model
            test_loss, accuracy = clients[idx].eval_test(clients[idx].net)
            print2log(f"test_loss:{test_loss},accuracy:{accuracy}")
            loss_list.append(test_loss)
            acc_list.append(accuracy)

            chosen_model_map[idx] = chosen_model_index
            end = time.time()
        ...
        chosen_model_map = np.array(chosen_model_map)
        for idx in range(args.n_clients):
            client_selection = np.where(chosen_model_map == chosen_model_map[idx])[0]
        ...
        # 用聚合模型代替现有模型
        for idx in range(args.n_clients):
            sever_model_pool[chosen_model_map[idx]].load_state_dict(w_servers[idx])
...
def set_log(args):
    ...
    if args.partition == "quantity_based_LDS":
        log_file_path = os.path.join(args.logdir, f"IFCA-label-{start_time}.log")
    else:
        log_file_path = os.path.join(args.logdir, f"IFCA-feature-{start_time}.log")
