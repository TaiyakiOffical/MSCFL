...
def run_task(args):
    ...
    while current_round != 0:
        ...
        if cluster_switch:
            para_lists = [
                np.concatenate(clients[idx].para_list) for idx in range(args.n_clients)
            ]
            begin_time = time.time()
            for i, row in enumerate(para_lists):
                print(f"Shape of row {i+1}: {np.shape(row)}")
            labels = kmeans_clustering(para_lists, clusters_num)
            end_time = time.time()
            ...
            total_clustering_time += end_time - begin_time
        # p2p聚合模型
        w_servers = []
        loss_list = []
        acc_list = []
...
def set_log(args):
    ...
    if args.partition == "quantity_based_LDS":
        log_file_path = os.path.join(args.logdir, f"One-shot-label-{start_time}.log")
    else:
        log_file_path = os.path.join(args.logdir, f"One-shot-feature-{start_time}.log")
