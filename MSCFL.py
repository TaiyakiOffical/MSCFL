# 主函数

# 按照每次比较的最低裁剪率去计算相同的值的数量在更小保留或裁剪之中所在的比例 分数在裁剪前计算
import torch
import datetime
import torch.nn as nn
import torch.nn.functional as F
import time
import random
from models import *
import numpy as np
from sklearn.cluster import KMeans
from sklearn.cluster import SpectralClustering
from sklearn.cluster import DBSCAN
from sklearn.cluster import AffinityPropagation
from sklearn.cluster import AgglomerativeClustering
from client import Client_Sub_S


import logging
from training import train_net
from miscellaneous import get_args, print2log
from data_process import partition_data


import copy
import os

args = get_args()
if args.prune_method == "weightsPrune":
    import weightsPrune as Prune
if args.prune_method == "filterPrune":
    import filterPrune as Prune
if args.prune_method == "fedDsePrune":
    import fedDsePrune as Prune


def check_correct_cluster(idx, client_selection, correct_cluster_group):
    if all(elem in client_selection for elem in correct_cluster_group[idx]):
        for i in range(len(correct_cluster_group)):
            if correct_cluster_group[i] != correct_cluster_group[idx] and all(
                elem in client_selection for elem in correct_cluster_group[i]
            ):
                return False
        return True
    else:
        return False


def hamming_distance(list1, list2):
    distance = 0
    for i in range(len(list1)):
        if list1[i] != list2[i]:
            distance += 1
    return distance


def bit_count(num):
    count = 0
    while num:
        num &= num - 1  # 清除最低的 1
        count += 1
    return count


def mask_to_bin(mask):
    # 将掩码数组转换为二进制字符串
    binary_string = "".join(str(bit) for bit in mask)

    # 将二进制字符串转换为整数
    binary_number = int(binary_string, 2)
    return binary_number


def flatten_score(score):
    flatten_score = np.concatenate(score)
    return flatten_score


def flatten_mask(mask):
    mfc_cal = []
    for tensor in mask:
        flatten_param = tensor.flatten()
        mfc_cal.append(flatten_param)
    flatten_mfc_cal = np.concatenate(mfc_cal)
    return flatten_mfc_cal


def flatten_mask2(mask):
    mfc_cal = []
    for tensor in mask:
        flatten_param = tensor.flatten()
        mfc_cal.append(flatten_param)
    return mfc_cal


def run_task(args):
    print2log("---------------Start Main Task--------------------")
    clusters_num = int(args.class_num / args.target_kinds)
    amount_in_cluster = int(args.n_clients / clusters_num)
    hamming_distance_matrix = [[] for i in range(args.n_clients)]
    mask_test_fc = [[] for i in range(args.n_clients)]
    prune_ratio = np.random.normal(
        args.prune_ratio_mean, args.prune_ratio_std, args.n_clients
    )
    prune_ratio = np.clip(prune_ratio, 0.2, 0.5)
    print2log(f"prune_ratio:{prune_ratio}")
    score_matrix_total = np.zeros((args.n_clients, args.n_clients))
    clustering_switch = True
    model_pool, net_glob, data_size = init_net(args)
    cfg_before = model_pool[0].cfg
    party_dataloader_train, party_dataloader_test, party_dataSize_dict = partition_data(
        args.dataset,
        args.datadir,
        args.partition,
        args.n_clients,
        args.batch_size,
        noise=args.noise,
        poisoning_type=args.poisoning_type,
        target_kinds=args.target_kinds,
        seed=args.seed,
        beta=args.beta,
        alpha=args.partition_alpha,
    )
    print2log("---------------Data Partition Complete--------------------")
    mask_ch_init = Prune.make_init_mask_ch(net_glob)
    mask_fc_init = Prune.make_init_mask_fc(net_glob)
    print2log("---------------Mask Init Complete--------------------")
    clients = []
    for idx in range(args.n_clients):
        clients.append(
            Client_Sub_S(
                idx,
                model_pool[idx],
                args.batch_size,
                args.epochs,
                args.first_train_ep,
                args.first_train_ep_ratio,
                args.lr,
                args.momentum,
                args.device,
                copy.deepcopy(mask_ch_init),
                copy.deepcopy(mask_fc_init),
                args.cfg_prune,
                args.in_ch,
                args.ks,
                args,
                party_dataloader_train[idx],
                party_dataloader_test[idx],
                data_size,
                score_before_prune=True,
            )
        )
    print2log("---------------Clients Init Complete,Start to Train--------------------")
    current_round = args.total_rounds
    total_clustering_time = 0
    client_selection_matrix = []
    correct_cluster_group = [
        [i, i + 1] for i in range(0, args.n_clients, 2) for _ in range(2)
    ]
    while current_round != 0:
        correct_cluster_number = 0
        begin_round_time = time.time()
        mask_total_ch = []
        mask_total_fc = []
        mask_total_ch_notFlatten = []
        mask_total_fc_notFlatten = []
        masks_ch = []
        masks_fc = []

        # 客户端开始训练
        for idx in range(args.n_clients):
            print2log(f"client{idx} starts to train")
            if current_round == args.total_rounds:
                clients[idx].train_firstRound(
                    prune_ratio[idx],
                    int(prune_ratio[idx] * 100),
                    copy.deepcopy(net_glob),
                    True,
                )
                # print(f"client{idx} score.shape:{np.array(clients[idx].score_fc).shape}")
            else:
                if (
                    args.swap_label == True
                    and current_round % 5 == 0
                    and random.random() < 0.3
                    and idx % 2 == 0
                    and idx < 8
                ):
                    # if current_round % 5 == 0 and random.random() < 0.3 and idx % 2 == 0 and idx < 8:
                    if args.partition == "quantity_based_LDS":
                        numbers = [x for x in range(idx + 2, args.n_clients, 2)]
                    else:
                        numbers = [x for x in range(idx + 1, args.n_clients, 2)]
                    idx2 = random.choice(numbers)
                    temp_ldr_train = copy.deepcopy(clients[idx].ldr_train)
                    temp_ldr_test = copy.deepcopy(clients[idx].ldr_test)
                    clients[idx].ldr_train = copy.deepcopy(clients[idx2].ldr_train)
                    clients[idx].ldr_test = copy.deepcopy(clients[idx2].ldr_test)
                    clients[idx2].ldr_train = temp_ldr_train
                    clients[idx2].ldr_test = temp_ldr_test
                    print2log(f"client{idx} has swapped the dataset with client{idx2}")
                clients[idx].train(
                    prune_ratio[idx],
                    int(prune_ratio[idx] * 100),
                    copy.deepcopy(net_glob),
                    True,
                )
            print2log(f"client{idx} finishes training")

        for idx in range(args.n_clients):
            mask_test_fc[idx].append(flatten_mask(clients[idx].mask_fc))

        #如果需要检测汉明距离则执行下面的语句段
        if current_round != args.total_rounds and args.swap_label == True:
            for i in range(args.n_clients):
                if len(mask_test_fc[i][-2]) != 0:
                    hamming_distance_matrix[i].append(
                        hamming_distance(mask_test_fc[i][-2], mask_test_fc[i][-1])
                    )
                    if (
                        len(hamming_distance_matrix[i]) >= 4
                        and hamming_distance_matrix[i][-1]
                        > args.hamming_distance_alpha * hamming_distance_matrix[i][-2]
                    ):
                        clients[i].net = copy.deepcopy(net_glob)
                        clients[i].mask_ch = copy.deepcopy(mask_ch_init)
                        clients[i].mask_fc = copy.deepcopy(mask_fc_init)
                        clients[i].train_firstRound(
                            prune_ratio[i],
                            int(prune_ratio[i] * 100),
                            copy.deepcopy(net_glob),
                            True,
                        )
                        clustering_switch = True
                        clients[i].net = None
                        print2log(f"client {i} needs to recluster")
        print2log(f"hamming_distance_matrix:{hamming_distance_matrix}")

        for idx in range(args.n_clients):
            if data_size != None:
                masks_ch.append(copy.deepcopy(clients[idx].get_mask_ch()))
            masks_fc.append(copy.deepcopy(clients[idx].get_mask_fc()))

        #初始化分簇操作所需的数据
        if clustering_switch == True:
            for idx in range(args.n_clients):
                if data_size != None:
                    mask_total_ch_notFlatten.append(flatten_mask2(clients[idx].mask_ch))
                mask_total_fc_notFlatten.append(flatten_mask2(clients[idx].mask_fc))
                if data_size != None:
                    mask_total_ch.append(clients[idx].mch.numpy())
                mask_total_fc.append(flatten_mask(clients[idx].mask_fc))

                if current_round != args.total_rounds:
                    mask_test_fc[idx].append([])
            
            #初始化分数矩阵
            score_matrix_fc = np.zeros((args.n_clients, args.n_clients))
            mask_fc_sum = [sum(mask_total_fc[i]) for i in range(args.n_clients)]
            bin_fc = [
                mask_to_bin(mask_total_fc[i].astype(int).tolist())
                for i in range(args.n_clients)
            ]
            length_fc = len(mask_total_fc[0])
            fc_mask_value = 2**length_fc - 1
            if data_size != None:
                score_matrix_ch = np.zeros((args.n_clients, args.n_clients))
                mask_ch_sum = [sum(mask_total_ch[i]) for i in range(args.n_clients)]
                bin_ch = [
                    mask_to_bin(mask_total_ch[i].astype(int).tolist())
                    for i in range(args.n_clients)
                ]
                length_ch = len(mask_total_ch[0])
                ch_mask_value = 2**length_ch - 1

            begin = time.time()
            print2log(f"begin:{begin}")

            #开始计算分数矩阵
            for idx1 in range(args.n_clients):
                for idx2 in range(args.n_clients):
                    if idx1 != idx2:
                        if data_size != None:
                            if max(mask_ch_sum[idx1], mask_ch_sum[idx2]) < max(
                                length_ch - mask_ch_sum[idx1],
                                length_ch - mask_ch_sum[idx2],
                            ):
                                # temp = np.array([1 if client_idx1_mask_ch[i] == client_idx2_mask_ch[i] == 1 else 0  for i in range(length_ch)])
                                temp = bin_ch[idx1] & bin_ch[idx2]
                                score = temp.bit_count() / min(
                                    mask_ch_sum[idx1], mask_ch_sum[idx2]
                                )
                            else:
                                temp = (bin_ch[idx1] ^ ch_mask_value) & (
                                    bin_ch[idx2] ^ ch_mask_value
                                )
                                score = temp.bit_count() / min(
                                    length_ch - mask_ch_sum[idx1],
                                    length_ch - mask_ch_sum[idx2],
                                )
                            score_matrix_ch[idx1][idx2] = score
                        if max(mask_fc_sum[idx1], mask_fc_sum[idx2]) < max(
                            length_fc - mask_fc_sum[idx1], length_fc - mask_fc_sum[idx2]
                        ):
                            temp = bin_fc[idx1] & bin_fc[idx2]
                            score = temp.bit_count() / min(
                                mask_fc_sum[idx1], mask_fc_sum[idx2]
                            )
                        else:
                            temp = (bin_fc[idx1] ^ fc_mask_value) & (
                                bin_fc[idx2] ^ fc_mask_value
                            )
                            score = temp.bit_count() / min(
                                length_fc - mask_fc_sum[idx1],
                                length_fc - mask_fc_sum[idx2],
                            )
                        score_matrix_fc[idx1][idx2] = score
                    if idx1 == idx2:
                        score_matrix_fc[idx1][idx2] = 1
                        score_matrix_ch[idx1][idx2] = 1
            if data_size != None:
                score_matrix_total = score_matrix_ch + score_matrix_fc
                print2log(f"score_matrix_ch:{score_matrix_ch}")
            else:
                score_matrix_total = score_matrix_fc + score_matrix_fc
            clustering_switch = False
            print2log(f"score_matrix_fc:{score_matrix_fc}")
            print2log(f"score_matrix_total:{score_matrix_total}")

            # 确定聚合方法并开始聚合
            client_selection_matrix = []
            if args.cluster_method == "agglomerative":
                labels = (
                    AgglomerativeClustering(
                        clusters_num, metric="precomputed", linkage="complete"
                    )
                    .fit((2 - score_matrix_total) / 2)
                    .labels_
                )
            if args.cluster_method == "kmeans":
                labels = (
                    KMeans(n_clusters=clusters_num, random_state=0)
                    .fit(score_matrix_total)
                    .labels_
                )
            if args.cluster_method == "spectral":
                labels = SpectralClustering(
                    n_clusters=clusters_num, affinity="precomputed", random_state=42
                ).fit_predict(score_matrix_total)
            if args.cluster_method == "affinityPropagation":
                labels = (
                    AffinityPropagation(affinity="precomputed", random_state=42)
                    .fit(score_matrix_total)
                    .labels_
                )
            end = time.time()
            total_clustering_time += end - begin

            for idx in range(args.n_clients):
                client_selection = np.where(labels == labels[idx])[0]
                client_selection_matrix.append(client_selection)
                if args.check_cluster_acc and check_correct_cluster(
                    idx, client_selection.tolist(), correct_cluster_group
                ):
                    correct_cluster_number += 1
            print2log(f"client_selection_matrix:{client_selection_matrix}")
            print2log(f"total_clustering_time:{total_clustering_time}")

        # 开始聚合模型
        w_servers = []
        loss_list = []
        acc_list = []
        for idx in range(args.n_clients):
            if clients[idx].net != None:
                w_clients = [clients[idx].get_state_dict()]
                if data_size != None:
                    temp_masks_ch = [masks_ch[idx]]
                temp_masks_fc = [masks_fc[idx]]
            else:
                w_clients = []
                if data_size != None:
                    temp_masks_ch = []
                temp_masks_fc = []
                for index in client_selection_matrix[idx]:
                    mask_test_fc[index].append([])
            for index in client_selection_matrix[idx]:
                index = int(index)
                if index != idx and clients[index].net != None:
                    w_clients.append(clients[index].get_state_dict())
                    if data_size != None:
                        temp_masks_ch.append(masks_ch[index])
                    temp_masks_fc.append(masks_fc[index])
            if len(w_clients) == 0:
                w_server = copy.deepcopy(net_glob.state_dict())
            else:
                if data_size != None:
                    w_server = clients[idx].Sub_FedAVG_S(
                        copy.deepcopy(net_glob.state_dict()),
                        w_clients,
                        temp_masks_ch,
                        temp_masks_fc,
                        copy.deepcopy(net_glob),
                        args.ks,
                        args.in_ch,
                    )
                else:
                    w_server = clients[idx].Sub_FedAVG_U(
                        copy.deepcopy(net_glob.state_dict()), w_clients, temp_masks_fc
                    )
            w_servers.append(w_server)
        current_round -= 1
        end_round_time = time.time()
        print2log(f"total_round_time = {end_round_time - begin_round_time}")

        # 用聚合模型代替现有模型
        for idx in range(args.n_clients):
            clients[idx].net = copy.deepcopy(net_glob)
            clients[idx].net.load_state_dict(w_servers[idx])
            test_loss, accuracy = clients[idx].eval_test(clients[idx].net)
            print2log(f"client{idx} test_loss:{test_loss},accuracy:{accuracy}")
            loss_list.append(test_loss)
            acc_list.append(accuracy)
        print2log(f"acc_mean:{sum(acc_list)/len(acc_list)}")
        print2log(f"loss_mean:{sum(loss_list)/len(loss_list)}")
        print2log(f"cluster accuracy:{correct_cluster_number/args.n_clients}")


def set_log(args):
    start_time = datetime.datetime.now().strftime("%Y-%m-%d %H-%M")
    if args.partition == "quantity_based_LDS":
        log_file_path = os.path.join(args.logdir, f"MSCFL-label-{start_time}.log")
    else:
        log_file_path = os.path.join(args.logdir, f"MSCFL-feature-{start_time}.log")
    if not os.path.exists(args.logdir):
        # 如果路径不存在，则创建路径
        os.makedirs(args.logdir)
    log_mode = logging.INFO
    logging.basicConfig(
        filename=log_file_path,
        format="%(asctime)s %(levelname)-8s %(message)s",
        datefmt="%m-%d %H:%M",
        level=log_mode,
        filemode="a",
    )


def Main():
    np.set_printoptions(threshold=np.inf)
    set_log(args)
    args.cfg_prune = (
        [1, "M", 1, "M"]
        if args.dataset != "cifar100"
        else [1, "M", 1, "M", 1, "M", 1, "M"]
    )

    print2log(args.dataset + " " + args.partition)
    run_task(args)
    return


if __name__ == "__main__":
    Main()
