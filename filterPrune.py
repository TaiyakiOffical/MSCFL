        ...
        for m in model.modules():
            ...
            if isinstance(m, nn.Linear):
                ...
                if step == 0:
                    ...
                    new_mask = np.where(
                        abs(tensor) < percentile_value, 0, mask_fc[step]
                    )
                    # Apply new mask
                    mask_fc[step] = new_mask
                print2log(
                    f"np.sum(mask_fc[step])/len(mask_fc[step].flatten():{np.sum(mask_fc[step])/len(mask_fc[step].flatten())}"
                )
