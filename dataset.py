...
class Texas100(data.Dataset):
    def __init__(self, root, data_indexes,transform=None, target_transform=None, train=True):
        self.root = root
        self.data_indexes = data_indexes
        self.transform = transform
        self.train = train
        self.target_transform = target_transform
        self.features, self.labels = self.__build_truncated_dataset__()
        print(self.features.shape)
        
    def __build_truncated_dataset__(self):
        data = np.load(self.root + "/texas100.npz")
        ...
        if self.train:
            return X_train[self.data_indexes], y_train[self.data_indexes]
        else:
            return X_test[self.data_indexes], y_test[self.data_indexes]
...
class Purchase100(data.Dataset):
    ...
    def __build_truncated_dataset__(self):
        data = np.load(self.root + "/purchase100.npz")
        features = data['features']
        labels = data['labels']
        X_train, X_test, y_train, y_test = train_test_split(features, labels, test_size=0.2, random_state=42)
        X_train = torch.tensor(X_train, dtype=torch.float32)
...
class MNIST_truncated(data.Dataset):
    def __init__(self, root, data_indexes=None, train=True, transform=None, target_transform=None, download=False):
        self.root = root
        self.data_indexes = data_indexes
        self.train = train
        self.transform = transform
        ...
        # type(data)为torch.tensor
        self.data, self.targets = self.__build_truncated_dataset__()

    def __build_truncated_dataset__(self):
        MNIST_obj = MNIST(self.root, self.train, self.transform, self.target_transform, self.download)
        MINST_data = MNIST_obj.data
        MINST_targets = MNIST_obj.targets

        if self.data_indexes is not None:
            MINST_data = MINST_data[self.data_indexes]
            MINST_targets = MINST_targets[self.data_indexes]

        return MINST_data, MINST_targets
    ...
    def __getitem__(self, index):
        ...
        # print(type(img), img.dtype, type(target), target.dtype)
...
class FMNIST_truncated(data.Dataset):
    ...
    def __build_truncated_dataset__(self):
        FMNIST_obj = FashionMNIST(self.root, self.train, self.transform, self.target_transform, self.download)
        FMNIST_data = FMNIST_obj.data
        FMNIST_targets = FMNIST_obj.targets

        if self.data_indexes is not None:
            
            FMNIST_data = FMNIST_data[self.data_indexes]
            FMNIST_targets = FMNIST_targets[self.data_indexes]
...
class SVHN_truncated(data.Dataset):
    ...
    def __build_truncated_dataset__(self):
        if self.train is True:
            ...
            SVHN_obj = SVHN(self.root, 'train', self.transform, self.target_transform, self.download)
            SVHN_data = SVHN_obj.data
            SVHN_targets = SVHN_obj.labels
        else:
            SVHN_obj = SVHN(self.root, 'test', self.transform, self.target_transform, self.download)
            SVHN_data = SVHN_obj.data
            SVHN_targets = SVHN_obj.labels

        if self.data_indexes is not None:
            self.data_indexes = self.data_indexes.astype(int)
            SVHN_data = SVHN_data[self.data_indexes]
            SVHN_targets = SVHN_targets[self.data_indexes]

        return SVHN_data, SVHN_targets
...
class CIFAR10_truncated(data.Dataset):
    ...
    def __init__(self, root, data_indexes=None, train=True, transform=None, target_transform=None, download=False):
        ...
        # type(data)为numpy.ndarray
        self.data, self.targets = self.__build_truncated_dataset__()

    def __build_truncated_dataset__(self):
        CIFAR10_obj = CIFAR10(self.root, self.train, self.transform, self.target_transform, self.download)
        CIFAR10_data = CIFAR10_obj.data
        CIFAR10_targets = np.array(CIFAR10_obj.targets)

        if self.data_indexes is not None:
            self.data_indexes = self.data_indexes.astype(int)
            CIFAR10_data = CIFAR10_data[self.data_indexes]
            CIFAR10_targets = CIFAR10_targets[self.data_indexes]
...
class FEMNIST(MNIST):
    ...
    def __init__(self, root, data_indexes=None, train=True, transform=None, target_transform=None,
                 download=False):
        super(MNIST, self).__init__(root, transform=transform,
                                    target_transform=target_transform)
        self.train = train
        ...
        if self.data_indexes is not None:
            self.data = self.data[self.data_indexes]
            self.targets = self.targets[self.data_indexes]
...
class CIFAR100_truncated(data.Dataset):
    ...
    def __build_truncated_dataset__(self):
        ...
        if self.data_indexes is not None:
            self.data_indexes = self.data_indexes.astype(int)
            data = data[self.data_indexes.astype(int)]
            target = target[self.data_indexes.astype(int)]
