...
def fake_prune_fc(percent, model, mask_ch, mask_fc, ks):
    ...
    """
    mask_activation = []
    for index, (key, value) in enumerate(model.activations.items()):
        if index == len(model.activations.keys()) - 1:
            pass
        else:
            # 对于中间层，统计的是所有样本中每个神经元的总激活值
            total_activation = model.activations[key]
            avg_activation = total_activation / model.sample_count

            # 生成掩码：保留激活值最高的50%神经元
            num_neurons = model.activations[key].size(0)
            num_keep = int(percent * 0.01 * num_neurons)

            # 找到激活值最大的索引
            _, sorted_indices = torch.topk(avg_activation, num_keep)
            mask_temp = torch.zeros(num_neurons)
            mask_temp[sorted_indices] = 1.0
            mask_activation.append(mask_temp)
    model.sample_count = 0
    for key, value in model.activations.items():
        model.activations[key] = 0
    if ks != None:
        step = 0
        score_fc = []
        ...
        for m in model.modules():
            ...
            if isinstance(m, nn.Linear):
                tensor = m.weight.data.clone().cpu().numpy()
                print()
                if step == 0:
                    ...
                    #     ix = idx0[i]
                    #     temp_mask[:, i * ks * ks : i * ks * ks + ks * ks] = mask_fc[
                    #         step
                    #     ][:, ix * ks * ks : ix * ks * ks + ks * ks]
                    ...
                    # for i in range(len(idx0)):
                    #     ix = idx0[i]
                    #     mask_fc[step][:, ix * ks * ks : ix * ks * ks + ks * ks] = (
                    #         new_mask[:, i * ks * ks : i * ks * ks + ks * ks]
                    #     )
                    mask_fc[0][~np.array(mask_activation[0], dtype=bool), :] = 0
                elif step == len(model.activations.keys()) - 1:
                    mask_fc[step][
                        :, ~np.array(mask_activation[step - 1], dtype=bool)
                    ] = 0
                else:
                    mask_fc[step] = np.outer(
                        mask_activation[step], mask_activation[step - 1]
                    )

                step += 1

                # tensor_list = tensor.reshape(-1).tolist()
        ...
        for m in model.modules():
            ...
            if isinstance(m, nn.Linear):
                ...
                step += 1
    return mask_fc, score_fc


# def fake_prune_fc_twice(percent, model, mask_ch, mask_fc, ks):
#     '''
#     This function derives the new pruning mask of fc layers, it put 0 for the weights under the given percentile
...
#                 new_mask[abs(tensor) < percentile_value] = 0
#                 # new_mask = np.where(abs(tensor) < percentile_value, 0, temp_mask)
#                 for i in range(len(idx0)):
#                     ix = idx0[i]
...
#             else:
#                 # new_mask = np.where(abs(tensor) < percentile_value, 0, mask_fc[step])
#                 print2log(len(np.where(abs(tensor) < percentile_value)[0])/len(tensor.flatten()))
#                 new_mask = np.ones_like(tensor)
#                 new_mask[abs(tensor) < percentile_value] = 0
#                 print2log(len(np.where(abs(tensor) < percentile_value)[0]))
#                 print2log(len(new_mask.flatten()))

#                 print2log(f"np.sum(new_mask)/len(new_mask.flatten()):{np.sum(new_mask)/len(new_mask.flatten())}")
#                 # Apply new mask
#                 mask_fc[step] = new_mask
#                 print2log(f"np.sum(mask_fc[step])/len(mask_fc[step].flatten():{np.sum(mask_fc[step])/len(mask_fc[step].flatten())}")
#             step += 1
#             tensor_list = tensor.reshape(-1).tolist()
#             min_val = min(tensor_list)
#             max_val = max(tensor_list)
...
def fake_prune_ch(percent, model, cfg_prune, ds):
    """
    ...
    for k, m in enumerate(model.modules()):
        if isinstance(m, nn.BatchNorm2d):
            ...
            if size > cfg_prune[idx_cfg]:
                _mask = weight_copy.abs().greater_equal(thre[step]).float().cuda()
            ...
            normalized_list = np.array(
                [(x - min_val) / (max_val - min_val) for x in tensor_list]
            )
    ...
    return cfg, cfg_mask, mask, score_ch
...
def real_prune_ch(model, cfg, cfg_mask, ks, in_ch, device, args):
    ...
    **NOTE: Please note that this function creates a new model based on the new config and returns the new model
    ...
    if ks == None:
        return model
    if args.model == "lenet5" and args.dataset in ["cifar10", "svhn"]:
        newmodel = LeNetBN5Cifar(nclasses=10, cfg=cfg, ks=args.ks).to(args.device)
        ks = 5
    elif args.model == "lenet5" and args.dataset == "cifar100":
        newmodel = LeNetBN5Cifar100(nclasses=100, cfg=cfg, ks=args.ks).to(args.device)
        ks = 2
    elif args.model == "lenet5" and args.dataset in ["mnist"]:
        newmodel = LeNetBN5Mnist(cfg=cfg, ks=args.ks).to(args.device)
        ks = 4
    elif args.model == "lenet5" and args.dataset in ["fmnist"]:
        newmodel = LeNetBN5Fmnist(cfg=cfg, ks=args.ks).to(args.device)
        ks = 4
    layer_id_in_cfg = 0
    ...
    for [m0, m1] in zip(model.modules(), newmodel.modules()):
        if isinstance(m0, nn.BatchNorm2d):
            ...
            print(f"m1.weight.data.shape:{m1.weight.data.shape}")
            print(f"len:{len(idx0)}")
            ...
            w1 = m0.weight.data[
                :, idx0.tolist(), :, :
            ].clone()  # [6, 3, 5, 5] [out_channel, in_channel, k, k]
        ...
        for m in model.modules():
            ...
            if isinstance(m, nn.Linear):
                ...
                if step == 0:
                    ...
                    m.weight.data = torch.from_numpy(tensor * mask_fc[step]).to(
                        weight_dev
                    )
                step += 1
    else:
...
def print_pruning(model, net_glob, is_print=True):
    """
    ...
    if is_print:
        print2log(
            f"total pruned: ({100 * (total_param-remain) / total_param:3.2f}% pruned), "
    ...
    output = [
        total_pruning,
        total_channel_pruning,
        total_fc_pruning,
        total_net_channel_pruning,
